describe("Quiz", () => {
  ...
  describe("filterQuestionsByDifficulty() method", () => {
    ...
    it("should update the 'questions' array with the questions filtered by difficulty", () => {
      ...
      const difficultQuizQuestions = quiz.filterQuestionsByDifficulty(2);
      const chosenQuestionDificulty = [
        {
          text: "Question 2",
          choices: ["d", "e", "f"],
          answer: "d",
      ...
      expect(difficultQuizQuestions).toEqual(chosenQuestionDificulty);
    });
    ...
    it("should not change the 'questions' array if the 1st argument is not a number between 1 and 3", () => {
      ...
      const filteredQuestionsWrongType = quiz.filterQuestionsByDifficulty("3");
      expect(quiz.questions).toEqual(filteredQuestionsWrongType);

      // 4. Check if the questions array is still the same as the original (it hasn't been filtered)

      const newquiz = new Quiz(questions, 60, 60);
      const filteredQuestionsDifficult = newquiz.filterQuestionsByDifficulty(8);
      expect(questions).toEqual(filteredQuestionsDifficult);
    });
  });
  ...
  describe("averageDifficulty() method", () => {
    ...
    it("should return the average difficulty of the questions in the quiz", () => {
      ...
      const test = quiz.averageDifficulty();
