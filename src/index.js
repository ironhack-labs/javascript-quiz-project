...
*/
	...
	function showQuestion() {
		...
		let porcentQuestion = (counterQuestion / questions.length) * 100;
		progressBar.style.width = `${porcentQuestion}%`;

		// 3. Update the question count text
		// Update the question count (div#questionCount) show the current question out of total questions

		questionCount.innerText = `Question ${counterQuestion} of ${questions.length}`; //  This value is hardcoded as a placeholder
		...
		let newChoises = document.createElement("div");
		question.choices.forEach((choice) => {
			newChoises.innerHTML += `<input type="radio" name="choice" value="${choice}">
      <label>${choice}</label><br>`;
			choiceContainer.appendChild(newChoises);
		});
